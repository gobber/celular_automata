...
public class SIR {
	...
	public static void run ( float initInfected, int size, int c, int r, int iterations, float pc, float pd, float pn ) throws IOException {
		...
		List<StatesImplementation> modelSIR = new ArrayList<>();
		
		modelSIR.add( new StatesImplementation( new S() ) );
		
		modelSIR.add( new StatesImplementation( new I() ) );
		
		modelSIR.add( new StatesImplementation( new R() ) );
		...
		for( int it = 0 ; it < iterations ; it ++ ) {
			...
			for( int x = 0; x < size ; x++ ) {
				...
				for( int y = 0 ; y < size ; y++ ) {
					...
					modelSIR.get( states.getPixel( x, y  ) )
		...
		while( ++cont < iterations ) {
			...
			s3.add( cont, Report.getInstance().getDr( cont ) );
